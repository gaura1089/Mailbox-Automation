...
function Generate-RandomPassword {
    ...
    # remaining chars (total 14–16)
    $all = $lower + $upper + $numbers + $special
    $remainingLength = Get-Random -Minimum 10 -Maximum 12   # total 14–16
...
foreach($user in $Users){
    ...
    try {
        ...
        if ($IsCheckOnly) {
            Write-Host "ℹ Skipped creation (EmpCode check only mode)"
            continue
        }
